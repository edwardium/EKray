...
# Version: 1.9.4 (Full Functionality Restore)
# Author: Kaveh & Edward
...
main_menu() {
    while true; do
        clear
        print_header "🚀 EKray Panel v1.9.4 🚀"
...
install_reality_service() {
    ...
    echo -e "\n${C_YELLOW}Installing VLESS+Reality Service...${C_RESET}"
    ...
    REALITY_KEYS=$($SINGBOX_BIN_PATH generate reality-keypair)
    ...
    REALITY_INBOUND=$(jq -n \
        ...
        '{ "type": "vless", "tag": "vless-reality-in", "listen": "::", "listen_port": $port, "users": [], "tls": { "enabled": true, "server_name": $sni, "reality": { "enabled": true, "handshake": { "server": $sni, "server_port": 443 }, "private_key": $p_key, "short_id": $s_id } } }')

    tmp_json=$(mktemp); jq --argjson new_inbound "$REALITY_INBOUND" '.inbounds += [$new_inbound]' "$CONFIG_PATH" > "$tmp_json"
    sudo mv "$tmp_json" "$CONFIG_PATH"
    echo "$PUBLIC_KEY" | sudo tee "$PUB_KEY_PATH" > /dev/null

    echo -e "\n${C_GREEN}Reality inbound added. Restarting service...${C_RESET}"; sudo systemctl restart sing-box; sleep 1
    echo "Service status after installation:"
...
install_hysteria2_service() {
    ...
    echo -e "\n${C_YELLOW}Installing Hysteria2 Service...${C_RESET}"
    ...
    if [ ! -f "/etc/sing-box/self-signed.crt" ]; then
        ...
        sudo openssl req -x509 -nodes -newkey ec:<(openssl ecparam -name prime256v1) -keyout /etc/sing-box/self-signed.key -out /etc/sing-box/self-signed.crt -subj "/CN=localhost" -days 3650 &> /dev/null
    fi
    ...
    echo -e "\n${C_GREEN}Hysteria2 inbound added. Restarting service...${C_RESET}"; sudo systemctl restart sing-box; sleep 1
    echo "Service status after installation:"
    check_service_status
}

list_and_manage_users() {
     echo -e "\n${C_MAGENTA}User management is under development for multi-protocol support.${C_RESET}";
}
...
install_singbox() { if command -v sing-box &> /dev/null; then echo -e "\n${C_GREEN}sing-box is already installed.${C_RESET}"; return; fi; echo -e "\n${C_YELLOW}Installing sing-box...${C_RESET}"; local ARCH=$(uname -m); case "$ARCH" in x86_64) ARCH="amd64" ;; aarch64) ARCH="arm64" ;; *) echo -e "${C_RED}Unsupported architecture${C_RESET}"; return 1 ;; esac; local LATEST_VERSION=$(curl -s "https://api.github.com/repos/SagerNet/sing-box/releases/latest" | jq -r '.tag_name'); if [ -z "$LATEST_VERSION" ]; then echo -e "${C_RED}Error getting latest version.${C_RESET}"; return 1; fi; echo "Latest version: $LATEST_VERSION"; local DOWNLOAD_URL="https://github.com/SagerNet/sing-box/releases/download/$LATEST_VERSION/sing-box-${LATEST_VERSION#v}-linux-${ARCH}.tar.gz"; echo "Downloading..."; if ! curl -sLo sing-box.tar.gz "$DOWNLOAD_URL"; then echo -e "${C_RED}Download failed.${C_RESET}"; return 1; fi; local EXTRACT_DIR="sing-box-${LATEST_VERSION#v}-linux-${ARCH}"; tar -xzf sing-box.tar.gz; sudo install -m 755 "${EXTRACT_DIR}/sing-box" "$SINGBOX_BIN_PATH"; sudo mkdir -p /etc/sing-box/; rm -rf "${EXTRACT_DIR}" sing-box.tar.gz; if [ ! -f "$SERVICE_PATH" ]; then create_service_file; fi; echo -e "${C_B_GREEN}sing-box core installed successfully.${C_RESET}"; }
...
system_status_check() { echo -e "\n${C_YELLOW}--- System Status Check ---${C_RESET}"; echo -n "1. Core: "; if [ -f "$SINGBOX_BIN_PATH" ]; then echo -e "${C_GREEN}Installed ($($SINGBOX_BIN_PATH version | awk '{print $3}'))${C_RESET}"; else echo -e "${C_RED}Not Found${C_RESET}"; fi; echo -n "2. Config Dir: "; if [ -d "/etc/sing-box" ]; then echo -e "${C_GREEN}Found${C_RESET}"; else echo -e "${C_RED}Not Found${C_RESET}"; fi; echo -n "3. Service: "; if [ -f "$SERVICE_PATH" ]; then SERVICE_STATUS=$(systemctl is-active sing-box); if [ "$SERVICE_STATUS" == "active" ]; then echo -e "${C_GREEN}Active (Running)${C_RESET}"; else echo -e "${C_RED}Inactive (Status: $SERVICE_STATUS)${C_RESET}"; fi; else echo -e "${C_RED}Not Found${C_RESET}"; fi; echo "---------------------------"; }
