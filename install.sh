...
# Version: 0.9.6 (Final Config Fix & Service Control)
# Author: Kaveh & Edward
...
show_main_menu() {
    clear
    echo "============================================="
    echo "         EKray Management Panel v0.9.6       "
...
service_management_menu() {
    ...
    echo "  6) Install Reality Service"
    echo "  7) Add Reality User"
    echo "  8) List Reality Users"
    echo "  9) Back to Main Menu"
    echo "---------------------------------------------"
    read -p "Enter your choice [1-9]: " service_choice

    case $service_choice in
        1) sudo systemctl start sing-box; echo "Service started." ;;
        2) sudo systemctl stop sing-box; echo "Service stopped." ;;
        3) sudo systemctl restart sing-box; echo "Service restarted." ;;
        4) check_service_status ;;
        5) view_service_logs ;;
        6) install_reality_service ;;
        7) add_reality_user ;;
        8) list_reality_users ;;
        9) return ;;
        *) echo -e "${RED}Invalid option.${NC}"; sleep 2; service_management_menu ;;
    esac

    if [[ "$service_choice" -ne 9 ]]; then
...
install_reality_service() {
    ...
    # Create config.json with the CORRECTED structure (no transport block)
...
EOF
    ...
    echo -e "${GREEN}Attempting to start the new service...${NC}"
    ...
    if systemctl is-active --quiet sing-box; then
        ...
        echo -e "${RED}Service failed to start. Please check the logs for errors.${NC}"
    fi
}
...
# --- All other functions (unchanged from v0.9.4) ---
install_singbox() {
    if command -v sing-box &> /dev/null; then
        echo -e "${GREEN}sing-box is already installed.${NC}"; return;
    fi
...
create_service_file() {
    echo -e "${YELLOW}Creating systemd service file...${NC}"; SERVICE_FILE_CONTENT="[Unit]\nDescription=sing-box service\nDocumentation=https://sing-box.sagernet.org\nAfter=network.target nss-lookup.target\n\n[Service]\nUser=root\nWorkingDirectory=/etc/sing-box\nCapabilityBoundingSet=CAP_NET_ADMIN CAP_NET_BIND_SERVICE\nAmbientCapabilities=CAP_NET_ADMIN CAP_NET_BIND_SERVICE\nExecStart=${SINGBOX_BIN_PATH} run -c ${CONFIG_PATH}\nRestart=on-failure\nRestartSec=10\nLimitNOFILE=infinity\n\n[Install]\nWantedBy=multi-user.target"; echo -e "$SERVICE_FILE_CONTENT" | sudo tee "$SERVICE_PATH" > /dev/null;
    sudo systemctl daemon-reload; sudo systemctl enable sing-box; echo -e "${GREEN}Service file created and sing-box service enabled.${NC}"
}
