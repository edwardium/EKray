...
show_main_menu() {
    clear
    echo "============================================="
    echo "         EKray Management Panel v1.0.0       "
...
service_management_menu() {
    ...
    echo "  4) View User Config"
    echo "  5) Delete Reality Service"
    echo " -------------------------------------------"
    ...
    echo "============================================="
    read -p "Enter your choice [1-12]: " service_choice
...
view_user_config() {
    ...
    list_reality_users
    echo ""
    read -p "Enter the user number to view details: " user_number

    if ! [[ "$user_number" =~ ^[0-9]+$ ]]; then
        echo -e "${RED}Invalid input. Please enter a number.${NC}"; return
    fi

    local user_line=$(sudo cat "$USER_DB_PATH" | sed -n "${user_number}p")

    if [ -z "$user_line" ]; then
        echo -e "${RED}User number '$user_number' not found.${NC}"; return
    fi

    local user_name=$(echo "$user_line" | cut -d: -f1)
    local user_uuid=$(echo "$user_line" | cut -d: -f2)

    generate_reality_link "$user_name" "$user_uuid" "no_clear"
}
...
generate_reality_link() {
    ...
    # Don't clear the screen if we're just viewing a config
    if [ "$3" != "no_clear" ]; then
        clear
    fi
...
system_status_check() { echo -e "${YELLOW}--- EKray System Status Check ---${NC}"; echo -n "1. Core: "; if [ -f "$SINGBOX_BIN_PATH" ]; then echo -e "${GREEN}Installed ($($SINGBOX_BIN_PATH version | awk '{print $3}'))${NC}"; else echo -e "${RED}Not Found${NC}"; fi; echo -n "2. Config: "; if [ -d "/etc/sing-box" ]; then echo -e "${GREEN}Found${NC}"; else echo -e "${RED}Not Found${NC}"; fi; echo -n "3. Service: "; if [ -f "$SERVICE_PATH" ]; then SERVICE_STATUS=$(systemctl is-active sing-box); if [ "$SERVICE_STATUS" == "active" ]; then echo -e "${GREEN}Active (Running)${NC}"; else echo -e "${RED}Inactive (Status: $SERVICE_STATUS)${NC}"; fi; else echo -e "${RED}Not Found${NC}"; fi; echo "---------------------------------"; }
uninstall_ekray() { echo -e "${RED}WARNING: This will REMOVE ALL related files.${NC}"; read -p "Are you sure? (y/n): " confirm; if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then sudo systemctl stop sing-box &> /dev/null; sudo systemctl disable sing-box &> /dev/null; sudo rm -f "$SERVICE_PATH"; sudo rm -f "$SINGBOX_BIN_PATH"; sudo rm -rf "/etc/sing-box/"; sudo systemctl daemon-reload; echo -e "${GREEN}EKray and sing-box core uninstalled.${NC}"; else echo -e "${YELLOW}Uninstall cancelled.${NC}"; fi; }
delete_reality_service() { if [ ! -f "$CONFIG_PATH" ]; then echo -e "${YELLOW}No service config to delete.${NC}"; return; fi; echo -e "${RED}WARNING: This will stop the service and delete the current Reality config.${NC}"; read -p "Are you sure? (y/n): " confirm; if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then sudo systemctl stop sing-box; sudo rm -f "$CONFIG_PATH" "$USER_DB_PATH" "$PUB_KEY_PATH"; echo -e "${GREEN}Reality config deleted.${NC}"; else echo -e "${YELLOW}Deletion cancelled.${NC}"; fi; }
install_reality_service() { if [ -f "$CONFIG_PATH" ]; then echo -e "${RED}A config file already exists. Delete it first.${NC}"; return; fi; echo -e "${YELLOW}Installing VLESS+Reality Service...${NC}"; read -p "Enter listen port: " listen_port; read -p "Enter SNI domain: " server_name; echo "Generating Reality key pair..."; REALITY_KEYS=$(/usr/local/bin/sing-box generate reality-keypair); PRIVATE_KEY=$(echo "$REALITY_KEYS" | grep "PrivateKey" | awk '{print $2}' | tr -d '",'); PUBLIC_KEY=$(echo "$REALITY_KEYS" | grep "PublicKey" | awk '{print $2}' | tr -d '",'); first_user_uuid=$(/usr/local/bin/sing-box generate uuid); first_user_name="initial-user"; sudo bash -c "cat > $CONFIG_PATH" << EOF
...
list_reality_users() { if [ ! -f "$USER_DB_PATH" ]; then echo -e "${RED}No users found.${NC}"; return; fi; echo -e "${YELLOW}--- List of Users ---${NC}"; i=1; while IFS=: read -r name uuid; do echo -e "  ${GREEN}${i})${NC} Name: ${YELLOW}${name}${NC}\n     UUID: ${uuid}\n--------------------------"; ((i++)); done < <(sudo cat "$USER_DB_PATH"); }
